...
client.connect_signal("request::titlebars", function(c)
    ...
    -- Buttons
        ...
        local close = create_title_button(c, "#FFA1A5", "#fcc7ca", ci(8, 8))
        close:connect_signal("button::press", function() c:kill() end)

        local float = create_title_button(c, "#FFEFB4", "#fcf3cc", ci(8, 8))
        float:connect_signal("button::press", function() c.floating = true end)

        local max = create_title_button(c, "#B2D0AC", "#d3edce", ci(8, 8))
    ...
    awful.titlebar(c, {
    ...
    }):setup{
        {
            {
                {
                    wibox.layout.margin(close, 20, 0, 2, 0), 
                    wibox.layout.margin(float, 0, 0, 2, 0), 
                    wibox.layout.margin(max, 0, 0, 2, 0), 
                    buttons = buttons,
